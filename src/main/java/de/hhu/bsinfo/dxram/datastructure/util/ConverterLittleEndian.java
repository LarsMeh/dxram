...
/**
 * Converter for primitive data types into an byte array and vice versa by using little endian.
 */
public final class ConverterLittleEndian {
    ...
    /**
     * Converts a byte array to a short. The index indicates from where the short will be read down.
     * It means if p_index is 4, the short will be read from index 4 and 3.
     *
     * @param p_arr   will be converted.
     * @param p_index indicates from where the short will be read down.
     * @return converted p_value.
     * @throws InvalidParameterException
     */
    @Contract(pure = true)
    public static short byteArrayToShort(@NotNull byte[] p_arr, int p_index) {
        if (p_arr.length < 2 || p_index < 1) throw new InvalidParameterException();
